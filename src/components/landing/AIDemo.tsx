...
import { Brain, Shield, Target, Zap, ChevronRight, MessageSquare } from "lucide-react";
...
export function AIDemo() {
  ...
  const demoSteps = [
    {
      type: "user",
      text: "Should I quit my job to start a specialized AI consultancy?",
      icon: MessageSquare
    },
    {
      type: "ai-header",
      text: "Analyzing Decision Logic...",
      icon: Brain
    },
    {
      type: "result",
    ...
    <div className="w-full max-w-2xl mx-auto min-h-[400px] flex flex-col justify-center gap-6 p-8 rounded-[2.5rem] glass-morphism relative overflow-hidden border border-white/5">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {demoSteps[step].type === "result" && (
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center ${demoSteps[step].color}`}>
                  {demoSteps[step].icon && (
                    <React.Fragment>
                      {React.createElement(demoSteps[step].icon, { size: 24 })}
                    </React.Fragment>
                  )}
                </div>
                <div>
                  <h4 className="text-lg font-medium tracking-tight">{demoSteps[step].title}</h4>
